...
TEST_CASE("MachineStatusRepo: Basic Type-Safe CRUD", "[HAL][MachineStatus]")
{
    ...
    {
        repo.setValue<bool>("valve", true);
        repo.setValue<int32_t>("motorPosition", -155);
        repo.setValue<uint32_t>("forceSensor", 990);

        auto valve = repo.getValue<bool>("valve");
        REQUIRE(valve.has_value());
        REQUIRE(valve.value() == true);

        auto motorPos = repo.getValue<int32_t>("motorPosition");
        REQUIRE(motorPos.has_value());
        REQUIRE(motorPos.value() == -155);

        auto force = repo.getValue<uint32_t>("forceSensor");
...
TEST_CASE("MachineStatusRepo: Signal Anti-Spam Logic", "[HAL][MachineStatus]")
{
    ...
    QSignalSpy spy(&repo, &IMachineStatusRepo::s_machineValueChanged);
    REQUIRE(spy.isValid());

    SECTION("Emits exactly once on new insertion")
    {
        repo.setValue<bool>("InterlockSafe", true);
        REQUIRE(spy.count() == 1);
    ...
    {
        repo.setValue<int32_t>("temperature", 25); // Emits (count = 1)
        REQUIRE(spy.count() == 1);

        // Simulating MCU sending the same value rapidly
        repo.setValue<int32_t>("temperature", 25);
        repo.setValue<int32_t>("temperature", 25);
        ...
        repo.setValue<int32_t>("temperature", 26);
...
{
    MachineStatusRepo repo;
    repo.setValue<int>("stressCounter", 0);
    ...
    auto writerTask = [&]() {
        ...
        {
            repo.setValue<int>("stressCounter", i);
        }
    };
    ...
    auto readerTask = [&]() {
        ...
        {
            auto val = repo.getValue<int>("stressCounter");
