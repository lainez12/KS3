...
- **Development Environment:**
  ...
  - **DevContainer:** A standardized VS Code development environment provided as a Docker container. It includes pre-configured C++, Qt, and compilation tools. It is set up to forward the X11 socket for rendering the GUI on the host, provides privileged access to interface with Arduino USB serial ports (`/dev/tty*`), and allows mapping host directories for external SDKs and includes (like the MVS library).
- **Quality Analysis:**
...
- **Environnement de Développement :**
  ...
  - **DevContainer :** Un environnement de travail standardisé pour VS Code tournant dans un conteneur Docker. Contient C++, Qt et les outils de compilation préinstallés. Il est configuré pour rediriger le socket X11 (affichage de l'IHM sur l'hôte), fournir un accès privilégié pour communiquer avec les ports série USB des Arduinos (`/dev/tty*`), et permet de monter des dossiers locaux pour intégrer des SDK (ex : librairies MVS).
- **Analyse de Qualité :**
